...
pro.listen = function(port) {
  ...
  this.server.sockets.on('connection', function(socket) {
    ...
    logger.fatal('rpc server connected with remote client: socket: %j'
        , {id: socket.id, address: socket.handshake.address, headers: socket.handshake.headers});
    ...
    socket.on('message', function(pkg) {
      try {
        ...
        logger.error('rpc server process message error: %s', e.stack);
      }
    });

    socket.on('disconnect', function(reason) {
      logger.error('rpc server disconnect with remote client reason: %j, socket: %j'
          , reason, {id: socket.id, address: socket.handshake.address, headers: socket.handshake.headers});
        ...
        setInterval(function(){
            ...
            for( k in self.msgQueues){
                ...
                if(!self.sockets[k]){
                    ...
                    message += util.format('[%s:%s]:%s ', self.sockets[k].handshake.address.address,
                                           self.sockets[k].handshake.address.port, self.msgQueues[k].length);
                }
            }
            if( total > 0 ) {
                logger.info('msgQueues size bufferMsg: %j, total: %s, message: %s', self.bufferMsg, total, message);
            }
