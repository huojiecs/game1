...
pro.dispatch = function(tracer, serverId, msg, opts, cb) {
  ...
  if(!mailbox) {
    ...
    if(!lazyConnect(tracer, this, serverId, this.mailboxFactory, cb)) {
      ...
      logger.error(util.format('[pomelo-rpc] fail to find remote server: %j, tracer: %j, stack: %s', serverId, tracer, (new Error()).stack));
  ...
  var send = function(tracer, err, serverId, msg, opts) {
    ...
    mailbox.send(tracer, msg, opts, function() {
      ...
      if(!!send_err) {
        logger.error('[pomelo-rpc] fail to send message. serverId: %s, msg: %j', serverId, msg);
...
pro.connect = function(tracer, serverId, cb) {
  ...
  mailbox.connect(tracer, function(err) {
    ...
    logger.warn('[pomelo-rpc] mailbox connect to remote server: %s -> %s', self.opts.clientId, serverId);
    mailbox.on('close', function(id) {
      logger.warn('[pomelo-rpc] mailbox closed id: %s +> %s', self.opts.clientId, id);
...
var lazyConnect = function(tracer, station, serverId, factory, cb) {
  ...
  if(!server) {
    logger.error('[pomelo-rpc] unknown server: %s, station.servers: %j, tracer: %j', serverId, station.servers, tracer);
    return false;
  }
  if(!online || online !== 1) {
    logger.error('[pomelo-rpc] server is not online: %s, station.servers: %j', serverId, station.servers);
  }
...
var addToPending = function(tracer, station, serverId, args) {
  ...
  if(pending.length > station.pendingSize) {
    ...
    logger.warn('[pomelo-rpc] station pending too much for: %s, %s',  serverId, pending.length);
    return;
  }
  pending.push(args);
};
