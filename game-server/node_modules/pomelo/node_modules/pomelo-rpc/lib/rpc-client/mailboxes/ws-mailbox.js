...
pro.connect = function(tracer, cb) {
  ...
  this.socket.on('message', function(pkg) {
    try{
      ...
      logger.error('rpc client process message with error: %s', e.stack);
    }
  });

  this.socket.on('connect', function() {
    logger.fatal('rpc socket connected to remote server host: %s, port: %s', self.host, self.port);
  ...
  this.socket.on('disconnect', function(reason) {
    logger.error('rpc socket is disconnect, reason: %s, remote server host: %s, port: %s, socket sessionid: %j'
        , reason, self.host, self.port, self.socket.socket.sessionid);
...
pro.send = function(tracer, msg, opts, cb) {
  ...
  if(this.bufferMsg) {
    ...
    try{
      ...
      logger.error('rpc client faild to send message. pkg: %s error: %j', util.inspect(pkg, {depth: 10}), e.stack);
    }
  }
};
...
var setCbTimeout = function(mailbox, id, tracer, cb) {
  var timer = setTimeout(function() {
    logger.error('rpc request is timeout, id: %s, remote server host: %s, port: %s, msg: %j, mailbox.requests: %s'
