...
pro.connect = function (tracer, cb) {
    ...
    this.socket.on('error', function (err) {
        logger.error('rpc socket is error, remote server host: %s, port: %s, err: %s', self.host, self.port, err.stack);
        self.emit('close', self.id);
        utils.invokeCallback(cb, err);
    });

    this.socket.on('end', function () {
        logger.error('rpc socket is disconnect, reason: end');
    ...
    this.socket.on('close', function (had_error) {
        logger.error('rpc socket is disconnect had_error: %j', had_error);
        var reqs = self.requests, cb;
        for (var id in reqs) {
            cb = reqs[id];
            utils.invokeCallback(cb, tracer, new Error('disconnect with remote server.'));
...
pro.send = function (tracer, msg, opts, cb) {
    ...
    if (!this.connected) {
        tracer.error('client', __filename, 'send', 'tcp-mailbox not init');
        utils.invokeCallback(cb, tracer, new Error('tcp-mailbox is not init'));
        return;
    }

    if (this.closed) {
        tracer.error('client', __filename, 'send', 'mailbox has already closed');
        utils.invokeCallback(cb, tracer, new Error('tcp-mailbox has already closed'));
        return;
    }
