...
pro.send = function (reqId, route, msg, recvs, opts, cb) {
    logger.debug('[%s] send message reqId: %s, route: %s, msg: %j, receivers: %j, opts: %j', this.app.serverId, reqId,
                 route, msg, recvs, opts);
    var emsg = msg;
    if (this.encode) {
        ...
        if (msg instanceof Array) {
            emsg = [];
            for (var i in msg) {
                emsg.push(this.connector.encode(reqId, route, msg[i]));
            }
        } else {
...
var bindEvents = function (self, socket) {
    ...
    socket.on('error', function (err) {
        if( !!err && !!err.stack &&
            ...
            || err.stack.indexOf('ETIMEDOUT') != -1
            || err.stack.indexOf('EPIPE') != -1
            || err.stack.indexOf('EHOSTUNREACH') != -1
            || err.stack.indexOf('ended by the other') != -1)) {
            logger.warn('connector socket id: %j closed: %j remoteAddress: %j fail with error: %s', socket.id, closed, socket.remoteAddress, err.stack);
        }
        else {
            logger.error('connector socket id: %j closed: %j remoteAddress: %j fail with error: %s', socket.id, closed, socket.remoteAddress, err.stack);
        }
        if (closed) {
            return;
        }
    ...
    socket.on('message', function (msg) {
        ...
        } else if (!!self.connector && self.connector.decode) {
...
var handleMessage = function (self, session, msg) {
    ...
    if (!type) {
        logger.error('invalid route string. route : %j', msg);
        return;
    }
    self.server.globalHandle(msg, session.toFrontendSession(), function (err, resp, opts) {
        if (!msg.id) {
            if (resp) {
                logger.debug('try to response to a notify: %j, %j', msg.route, resp);
            }
            return;
        }
        if (!resp) {
