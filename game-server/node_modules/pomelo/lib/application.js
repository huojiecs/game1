...
Application.configureLogger = function(logger) {
  if (process.env.POMELO_LOGGER !== 'off') {
    ...
    if(fs.existsSync(presentPath)) {
      logger.configure(presentPath, {serverId: this.serverId, base: base, pid: process.pid});
    } else if(fs.existsSync(originPath)) {
      logger.configure(originPath, {serverId: this.serverId, base: base, pid: process.pid});
    } else {
...
Application.masterStop = function(force) {
    logger.fatal('masterStop received.')

    var _ = require('underscore');
    var self = this;

    var stopInterval = setInterval(function(){
        logger.fatal('servers online: %j', _.keys(self.servers));

        if( _.size(self.servers) == 0) {
            logger.fatal('[pomelo application] master will shutdown in 3 secondes...');
...
Application.stop = function(force) {
  ...
  var shutDown = function() {
    appUtil.stopComps(self.loaded, 0, force, function() {
      ...
      if(force) {
          logger.fatal('[pomelo application] application will shutdown in 1 secondes...');
          setTimeout(function(){
              process.exit(0);
          }, 1000);
      }
