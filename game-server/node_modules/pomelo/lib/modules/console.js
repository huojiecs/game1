...
var checkPort = function(server, cb) {
  ...
  var cmd = !!process.platform.match(/^win/) ? 'netstat -anp tcp | findstr /R "TCP[^:]*:%s"' : 'netstat -tln | grep %s';
  if (!utils.isLocal(host)) {
    cmd = 'ssh ' + host + ' ' + cmd;
  }

  cmd = util.format(cmd, p);

  exec(cmd, function(err, stdout, stderr) {
    if (stdout || stderr) {
      utils.invokeCallback(cb, 'busy', {stdout: stdout, stderr: stderr});
    } else {
      p = server.clientPort;
      exec(cmd, function(err, stdout, stderr) {
        if (stdout || stderr) {
...
var runServer = function(app, server, cb) {
  checkPort(server, function(status, message) {
    if(status === 'busy') {
      utils.invokeCallback(cb, new Error(util.format('Port occupied already, check your server to add: %j, message: %j', server, message)));
    } else {
