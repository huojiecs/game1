...
var handleHandshake = function(socket, pkg) {
    ...
    if(!pkg.body) {
        logger.warn('could not strdecode invalid data package. %j', pkg.type);
//        socket.disconnect();
    }
    else {
...
var handle = function(socket, pkg) {
  ...
  if(!!handler) {
      try {
          ...
          logger.error('fail to handler message pkg: %j error: %s', pkg, err.stack);
          socket.disconnect();
      }
  }	else {
    logger.warn('could not find handler invalid data package. %j', pkg);
    socket.disconnect();
  }
};
