...
Command.prototype.handle = function(socket) {
  if(!this.heartbeat) {
    ...
    logger.debug('received client %j heartbeat %j.', socket.id, socket.remoteAddress);
  ...
  this.heartbeats[socket.id] = setTimeout(function() {
    ...
    if(self.disconnectOnTimeout) {
      self.timeouts[socket.id] = setTimeout(function() {
        logger.warn('client %j heartbeat timeout remote: %j.', socket.id, socket.remoteAddress);
        socket.disconnect();
      }, self.timeout);
    }
  }, this.heartbeat);
