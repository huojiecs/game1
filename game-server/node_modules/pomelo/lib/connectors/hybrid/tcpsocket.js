...
var ondata = function(socket, chunk) {
  ...
  if(socket.state === ST_HEAD && socket.packageBuffer === null && socket.headOffset === 0 && !checkTypeData(chunk[0])) {
    logger.info('close the connection with invalid head message, the remote ip is %s && port is %s && message is %j', socket._socket.remoteAddress, socket._socket.remotePort, chunk);
    socket.close();
  }
