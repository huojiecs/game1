...
Switcher.prototype.newSocket = function(socket) {
  if(this.state !== ST_STARTED) {
    ...
    socket.recordAddress = socket.remoteAddress;
    socket.recordPort = socket.remotePort;

    socket.on('error', function (err) {
        logger.info('connector remote socket ip: %s, port: %s is error: %s', socket.recordAddress, socket.recordPort, err.stack);
//        self.emit('error', err);
    });
  ...
  if(!!this.timeout) {
    var timer = setTimeout(function() {
      logger.info('connection is timeout without communication, the remote ip is %s && port is %s', socket.recordAddress, socket.recordPort);
      socket.destroy();
    }, this.timeout * 1000);
