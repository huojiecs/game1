...
utils.waitingForRpc = function (name, exist, delay, callback) {

    callback = callback || delay;
    delay = _.isFunction(delay) ? 1 : delay;

    logger.fatal('waitingForRpc: begin waiting for rpc name: %s, exist: %s, delay: %s', name, exist, delay);

    if (exist && !!pomelo.app && !!pomelo.app.rpc && name in pomelo.app.rpc) {

        logger.fatal('waitingForRpc: %s is exist.', name);

        return utils.invokeCallback(callback);
    }

    if (!exist && (!pomelo.app || !pomelo.app.rpc || !(name in pomelo.app.rpc))) {
        logger.fatal('waitingForRpc: %s is not exist.', name);
        return utils.invokeCallback(callback);
    }

    var nextTry = function () {
        logger.fatal('waitingForRpc: waiting %ss for %s rpc exist:%d...', ++delay, name, exist);
        return setTimeout(function () {
            utils.waitingForRpc(name, exist, delay, callback);
        }, delay * 1000);
    };

    if (!exist) {

        var servers = pomelo.app.getServersByType(name);
        ...
        var jobs = _.map(servers, function (item) {
            return Q.ninvoke(pomelo.app.rpc[name][name + 'Remote'], 'ping', null, item.id);
        });

        return Q.timeout(Q.allSettled(jobs), 5000)
            .then(function (results) {
                      if (_.some(results, function (result) {
 ...
 * @param {object} rpc 远程调用
 * @param cb {function}
 * @return promise
 * */
utils.notifyStatus = function (serverType, status, delay, cb) {
    cb = cb || delay;
    delay = _.isFunction(delay) ? 1 : delay;

    var nextTry = function () {
        logger.fatal('notify status retry {%s} to {%s} : waiting %ds ...', status, serverType, ++delay);
        return setTimeout(function () {
            utils.notifyStatus(serverType, status, delay, cb);
        }, ++delay * 1000);
    };

    // 'servers' 用法是不对的， 没有引入 pomelo 的 Constant 所以直接 'servers'
    var servers = pomelo.app.get('servers');
    var conns = servers['connector'];
    var openConns = pomelo.app.getServersByType('connector');
    ...
    for (var i in conns) {
        var conn = conns[i];
        jobs.push(Q.ninvoke(pomelo.app.rpc[serverType][serverType + 'Remote'], 'notifyServerStatus', null, conn.id,
                            pomelo.app.getServerId(), status));
    }

    Q.all(jobs)
        .then(function () {
                  logger.fatal('%s notify status Ok %s to %s is Ok... ', pomelo.app.getServerId(), status, serverType);
                  utils.invokeCallback(cb);
              })
        .catch(function (err) {
                   logger.fatal('%s notify status failed %s to %s err: {%s}... ', pomelo.app.getServerId(), status,
                                serverType, utils.getErrorMessage(err));
                   nextTry();
               })
        .done();
};
 ...
 * 所有后端服务是都ok函数
 * */
utils.allServerIsOk = function () {
    var allServers = pomelo.app.getServers();
    var serverStatus = pomelo.app.get('serverStatus');
    if (null == allServers || null == serverStatus) {
        return false;
    }

    logger.info("allServerIsOk serverStatus: %j", serverStatus);

    var id;
    for (id in allServers) {
        if (id.indexOf('connector') !== -1 || id.indexOf('dbcache') !== -1 || id.indexOf('idip') !== -1
            || id.indexOf('Idip') !== -1) {
            continue;
        }
        if (!serverStatus[id]) {
...
/**
 * 延后调用函数 _.after 添加了一个 n次之前每次执行函数
 * @param {number} times 延后次数
 * @param {function} eFunc n次调用后执行函数
 * @param {function} bFunc n次之前执行函数
 * */
utils.after = function (times, eFunc, bFunc) {
    if (times <= 0) {
        return eFunc;
    }
    return function () {
        logger.info('exec after func times %d', times);
        if (--times < 1) {
            logger.info('exec after func times after ');
...
/**
 * 判断指定时间是否在指定的时间格式内
 * @param {string} timeStr 时间格式
 * @param {number} nowTime 指定时间
 * @return {boolean}
 * */
utils.isInTimeExpression = function (timeStr, nowSec) {
    if ((new TimeExpression(timeStr).isExpressionTime(nowSec))) {
        return true;
    }
    return false;
};
