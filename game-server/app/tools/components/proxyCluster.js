...
var Client = require('pomelo/node_modules/pomelo-rpc').client;
...
pro.start = function (cb) {
    ...
    var list = [];
    _.each(options, function (eachServers, eachServerType) {
        if (_.isArray(eachServers)) {
            _.each(eachServers, function (eachServer, s) {
                if (!!eachServer && eachServer.port > 0 && !!eachServer.host) {
                    list.push({
                                  id: eachServer.id,
                                  serverType: eachServerType,
                                  host: eachServer.host,
                                  port: eachServer.port
                              });
                }
            });
        }
    ...
    logger.debug('cluster addServers list: %j', list);

    this.addServers(list);
...
pro.afterStart = function (cb) {
    ...
    logger.fatal('afterStart');

    this.app.__defineGetter__('cluster', function () {
...
var getProxyRecords = function (app, sinfo) {
    ...
    record = getUserClusterPath(appBase, sinfo.serverType);
    if (record) {
...
var genRouteFun = function () {
    return function (session, msg, app, cb) {
        var routes = app.get('__routes_cluster__');
        ...
        var route = routes[type] || routes['default'];
