...
var makeCookie = function (path, accountType) {

    return {
        session_id: config.vendors.msdkPayment.cookie[accountType].session_id,
        session_type: config.vendors.msdkPayment.cookie[accountType].session_type,
        org_loc: urlencode(path),
        appip: '127.0.0.1'
    };
};

Handler.get_balance_m = function (params, accountType) {
    ...
    var msdkPayment = config.vendors.msdkPayment[config.vendors.tencent.platId];

    return apiWrapper.request(requires, params, makeCookie(path, accountType), function () {
        return apiWrapper.makeURL(msdkPayment.hostUrl, path, params,
                                  msdkPayment.appid,
                                  msdkPayment.appKey + '&');
    });
};

Handler.pre_transfer = function (params, accountType) {
...
Handler.confirm_transfer = function (params, accountType) {
...
Handler.cancel_transfer = function (params, accountType) {
...
Handler.pay_m = function (params, accountType) {
    var path = '/mpay/pay_m';
...
Handler.cancel_pay_m = function (params, accountType) {
...
Handler.present_m = function (params, accountType) {
    ...
    params.discountid = msdkPayment.discountid;
    params.giftid = msdkPayment.giftid;
 ...
 * 2	/v3/r/mpay/subscribe_m (订阅物品查询接口)
 ...
 extend: '' } ] }
 */
Handler.subscribe_m = function (params, accountType, cmd) {
    ...
    var cookie = makeCookie(path, accountType);
    ...
    var msdkPayment = config.vendors.msdkPayment['SYGZ'];
...
Handler.subscribe_m_QUERY = function (params, accountType) {
    var path = '/mpay/subscribe_m';
...
Handler.subscribe_m_PRESENT = function (params, accountType) {
    var path = '/mpay/subscribe_m';
