...
Handler.Init = function () {
    ...
    self.zsetName =
    config.redis.chart.zsetName + ':union@' + config.list.serverUid + ':' + pomelo.app.getMaster().port;
//    self.zsetZhanli =
...
Handler.GetChart = function (roleID, unionID, callback) {
    ...
    zRevRank(self.zsetName, unionID)
        .then(function (rank) {
                  if (null != rank) {
                      chartID = (rank || 0) + 1;
                      result['unionRanking'] = chartID;
                  }
                  return zRevRange(self.zsetName, 0, defaultValues.chartUnionNumber - 1, 'WITHSCORES');
              })
