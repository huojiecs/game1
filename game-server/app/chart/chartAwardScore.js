        ...
        zremrangebyrank(self.zsetName, 0, -1)
            .then(function (result) {
                      ...
                      return Q.until(function () {
                          ...
                          var lastRoleID = 0;
                          return Q.until(function () {
                              return Q.ninvoke(chartSql, 'ChartLoadAwardScore', dbLoopCount, lastRoleID)
                                  .then(function (players) {
                                            logger.info("ChartLoadAwardScore load player: %s", players.length);
                                  ...
                                  .catch(function (err) {
                                             logger.error("ChartLoadAwardScore failed: %s", utils.getErrorMessage(err));
                                             return true;
                                         });
                          }).then(function (each) {
...
Handler.UpdatePlayerScore = function (roleInfo, callback) {
    return this.AddPlayerScore(roleInfo.roleID, roleInfo.awardScore || 0, callback);
};
