/**
 * The file chartAres.js Create with WebStorm
 * @Author        gaosi
 * @Email         angus_gaosi@163.com
 * @Date          2014/12/8 18:07:00
...
Handler.Init = function () {
    ...
    /** 设置数据key 类似于表名*/
    self.zsetName =
    config.redis.chart.zsetName + ':ares@' + config.list.serverUid + ':' + pomelo.app.getMaster().port;
    self.redisAresInfo =
    config.redis.chart.zsetName + ':aresInfo@' + config.list.serverUid + ':' + pomelo.app.getMaster().port;
    self.redisRoleInfo =
        ...
        Q.all(jobs)
            .then(function () {
                      ...
                      return Q.until(function () {
                          ...
                          /** 对一个数据库下所有数据读取*/
                          return Q.until(function () {
                              return Q.ninvoke(chartSql, 'ChartLoadAres', dbLoopCount, lastRoleID)
                                  .then(function (players) {

                                            /** 一次读取100个玩家数据*/
                                            self.AddPlayers(players);
                                            logger.info("ChartLoadAres load player: %s", players.length);
                                  ...
                                  .catch(function (err) {
                                             logger.error("ChartLoadAres failed: %s", utils.getErrorMessage(err));
                                             return true;
                                         });
                          }).then(function (each) {
                              /** 所有数据库读取完毕结束条件*/
...
Handler.AddPlayers = function (players, callback) {
    var self = this;
    logger.info("Ares Try to AddPlayers count: %d", players.length);
    ...
    _.each(players, function (player) {
        ...
        if (0 != rankKey) {
            ...
            multi.hset(self.redisAresInfo, roleID, value);
        }
    });
...
Handler.RemovePlayerScore = function (roleID, callback) {
    ...
    var jobs = [
        ...
        hdel(self.redisAresInfo, roleID)
    ];
...
Handler.GetChart = function (roleID, callback) {
    ...
    Q.all(jobs)
        ...
        .then(function (roleIDs) {
                  ...
                  return Q.all(jobs);
//                  return hmGet(self.redisAresInfo, keys);
              })
        .then(function (dates) {
                  var rank = 0;
                  var i = 0;
